...
export default function ExtensionsDashboard({ category, extensions, flagged, onAcknowledge, onRemove, onScan }: Props) {
  ...
  const filteredExtensions = searchQuery.trim() === ''
    ? sorted
    : sorted.filter(ext => ext.name.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  return (
    <div className="cat-dashboard">
      ...
      <div className="card">
        <div className="card-title">All Installed Extensions ({filteredExtensions.length})</div>
        ...
        {filteredExtensions.length > 0 ? (
          ...
          <div className="cat-events-empty">
            ...
            <p className="cat-events-clean">{searchQuery.trim() ? 'No matching extensions' : 'No extensions installed'}</p>
          </div>
        )}
      </div>
